pipeline {
    ...
    stages {
        stage('SCM Checkout'){
            steps {
            git branch: 'main', url: 'https://github.com/Kutty1021/Microservices.git'
            sh 'ls'
            }
        }
                        ...
                        script{
                            dir('wishlist-microservice-python') {
                                ...
                                withSonarQubeEnv('sonar-pro') {
                                    ...
                                    -D sonar.host.url=http://13.127.124.88:9000/"""
                                }
                            }
                        }
                    }
                ...
                parallel (
                    'docker login': {
                        withCredentials([string(credentialsId: 'dockerPass', variable: 'dockerPassword')]) {
                            sh "docker login -u kutty96 -p ${dockerPassword}"
                        }
                    },
                    'ui-web-app-reactjs': {
                        dir('ui-web-app-reactjs'){
                            sh """
                            docker build -t kutty96/ui:v1 .
                            docker push kutty96/ui:v1
                            docker rmi kutty96/ui:v1
                            """
                        }
                    },
                    'zuul-api-gateway' : {
                        dir('zuul-api-gateway'){
                            sh """
                            docker build -t kutty96/api:v1 .
                            docker push kutty96/api:v1
                            docker rmi kutty96/api:v1
                            """
                        }
                    },
                    'offers-microservice-spring-boot': {
                        dir('offers-microservice-spring-boot'){
                            sh """
                            docker build -t kutty96/spring:v1 .
                            docker push kutty96/spring:v1
                            docker rmi kutty96/spring:v1
                            """
                        }
                    },
                    'shoes-microservice-spring-boot': {
                        dir('shoes-microservice-spring-boot'){
                            sh """
                            docker build -t kutty96/spring:v2 .
                            docker push kutty96/spring:v2
                            docker rmi kutty96/spring:v2
                            """
                        }
                    },
                    'cart-microservice-nodejs': {
                        dir('cart-microservice-nodejs'){
                            sh """
                            docker build -t kutty96/ui:v2 .
                            docker push kutty96/ui:v2
                            docker rmi kutty96/ui:v2
                            """
                        }
                    },
                    'wishlist-microservice-python': {
                        dir('wishlist-microservice-python'){
                            sh """
                            docker build -t kutty96/python:v1 .
                            docker push kutty96/python:v1
                            docker rmi kutty96/python:v1
                            """
                        }
                    }
                )
